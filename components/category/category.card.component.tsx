...
export default async function CategoryCard({ name, name_en, img, query, index }: prop) {
    ...
    return (
        <Link className={` w-full h-full  relative *:w-full font-bold aspect-square rounded-lg bg-white p-8 ${index === 0 ? "col-span-4 row-span-2 md:col-span-2  text-[24px]" : " col-span-2 md:col-span-1 text-[16px]"}`} href={{ pathname: "/products", query }}>
