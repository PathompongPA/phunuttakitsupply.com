...
export default function Item({ children, dropdown = false, className, pathname, hash, query, variant, onClick }: prop) {
    ...
    return (
        <li className={style({ variant, className })}>
            ...
            <div className={`border-b-4 rounded-lg w-full ease-in-expo border-transparent origin-left group-hover/item:scale-100  ${!query?.category && path === pathname ? "scale-100 lg:border-orange" : "scale-0"}
