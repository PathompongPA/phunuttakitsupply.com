...
export default function Navbar({ category }: prop) {
    ...
    return (
        <nav className={`fixed lg:py-2 top-0 left-0 w-screen flex justify-center duration-600 ease-in-out ${lastScrollY === 0 ? " bg-transparent " : "bg-white"}  data-[hidden=true]:scale-0 origin-center data-[hidden=true]:-top-full z-50 `} aria-label="main navigation" data-hidden={hidden} data-position={lastScrollY} >
            <div className="flex flex-col lg:flex-row justify-between lg:items-center w-full max-w-7xl lg:px-8 relative " data-hidden={hidden}  >
