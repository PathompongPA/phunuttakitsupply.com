...
export default async function BlogContent({ id }: prop) {
    ...
    return (
        <div className=" w-screen min-h-screen  flex flex-col items-center overflow-hidden ">
            <div className=" max-w-5xl min-h-screen bg-gry-1 flex flex-col gap-8 w-full md:px-4 ">
                <div className=" flex flex-col gap-4 lg:gap-8 p-8 lg:p-8 items-center bg-gray-50 ">
                    <div className=" w-full  text-[14px] md:text-[12px] lg:text-[16px] prose max-w-none " dangerouslySetInnerHTML={{ __html: blog?.content }}></div>
                    <div className=" flex gap-2 justify-end w-full text-[12px] ">
                        <div className=" text-gray-5">{createAt}</div>
                        <div className=" text-gray-5 list-decimal"> Posted by {blog?.user_updated?.first_name}</div>
                    </div>
                </div>
            </div>
        </div>
