...
export default function BlogCard({ id, title, subtitle, img }: prop) {
    ...
    return (
        <Link className="relative border-b-4 border-orange aspect-square min-w-62.5 lg:w-full rounded-t-lg overflow-hidden" href={{ pathname }}>
            <div className="absolute bottom-0 left-1/2 gap-2  -translate-x-1/2 w-full p-4  z-10 text-white truncate flex flex-col *:w-full text-end">
                <span className=" truncate text-[16px] font-semibold" >{title}</span>
                <span className=" text-wrap text-[12px] max-h-[50px] overflow-hidden  text-gray-3">{subtitle}</span>
            </div>
